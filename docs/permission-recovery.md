...
**Quit & Reopen** uses the same quit policy as normal Quit, including History
finalization of pending captures. A failed finalization keeps the remaining
captures pending, cancels quit, and clears the reopen intent. Relaunch preparation
starts only after every pending capture has committed. Before accepting
