...
81. **Editable marks (implementer, 2026-09-25, ticket 84; decisions 54, 57, 59, 61 and 77):**
    - **Where:** `MarkEditor` in FrisketCore (`MarkEditing.swift`) holds the selected mark and makes every change: hit-testing, move, resize, delete and restyle, plus Tab order and each mark's VoiceOver label. It works through `UndoableEdits.apply(_:named:)`, so each change is one undo step named "Move Arrow", "Resize Shape", "Delete Solid Redaction", "Restyle Arrow" and so on. `EditorWindow` only forwards pointer and key events (`MarkKey`) and draws the outline and handles it reports.
    - **Marks:** Solid redactions, Blur and Magnify boxes, shapes, arrows and labels; a mark is its index in its list (`MarkReference`), so an undo that removes the selected mark clears the selection. Crop is not a mark. Hits go topmost first in paint order; a shape is hit on its outline and an arrow on its line, so a click inside an outline reaches what is under it. Boxes have four corner handles (the opposite corner stays put), an arrow its tail and tip, a label none until ticket 86 gives it a size. A change that would leave an empty mark is refused and registers nothing.
    - **Selecting:** a new Select tool (V, first in the toolbar; Solid Redaction stays the tool the editor opens with). With any tool, a click without a drag on a mark selects it, and a press on the selected mark's body or handle moves or resizes it; with the Select tool a press on any mark does. A click elsewhere or Esc deselects; Esc with nothing selected keeps its old meaning. Newly drawn marks are not selected, so drawing over a mark still draws.
...
82. **Solid redaction palette (implementer, 2026-09-25, ticket 88; decisions 54, 57, 61 and 81):**
...
83. **Arrow styles and the Line tool (implementer, 2026-09-25, ticket 85; decisions 54, 57, 59, 68 and 81):**
    - **Model:** an arrow annotation carries an `ArrowStyle` (Standard, Curved, Double, Line) and an `ArrowBend`. The bend places the curve's middle handle relative to the chord: `along` times the chord plus `across` times the chord turned a quarter, so moving the arrow or dragging its tail or tip keeps its shape. A new Curved arrow bows by a fifth of its length (`across = −0.2`); every other style ignores the bend. Line is an arrow-kind mark with no head, so select, move, resize, restyle and undo work unchanged; its noun is "Line".
    - **Geometry in the core:** `ArrowGeometry` (FrisketCore) turns a mark into filled polygons, in output pixels, and `AnnotationPainter` only fills them. Standard: a shaft tapering from half the line width at the tail to one and a half times it, under a solid triangular head (length `(6 + 3w)` pt, half as wide on each side as it is long, shortened to 60% of a short arrow). Curved: a quadratic Bézier through the middle handle (control point `2M − (tail + tip)/2`), sampled at 48 points, same taper; the head lies along the curve's tangent at the tip. Double: a shaft of constant width with a head at each end, each at most 40% of the length. Line: a constant-width bar with square ends, the old pen. This supersedes decision 68's open two-wing arrow head.
    - **Plate:** each polygon is filled white and outlined 2 px in white first (1 px outside), then filled in ink, so decision 68's order (plates, redactions again, ink) holds.
    - **Handles:** a Curved arrow has a third handle, `.bend`, at the curve's midpoint; dragging it is one undo step ("Resize Arrow"). Hit-testing and the bounds follow the sampled curve.
    - **Tolerance:** geometry goldens are exact to 1e-9 in the polygons' own units. Pixel goldens check pixels at least 1 px inside or outside a polygon edge, which must be exactly ink or exactly the base; edge pixels are antialiased and not compared. Preview and delivered image are compared byte for byte (tolerance 0).
    - **Controls:** the toolbar gains a Line tool (L) and an Arrow style menu (Standard, Curved, Double). The style menu and the line-width menu set the active tool's style and width for new marks, and restyle the selected mark when there is one. Both are labelled for VoiceOver.

84. **Labels typed on the image (implementer, 2026-09-25, ticket 86; decisions 54, 57, 59, 68, 77, 81 and 83):**
    - **Model:** a text annotation carries a `LabelFormat`: a `LabelStyle` (Standard, Outlined, Box), a size in document points (the menu offers 14, 18, 24, 36 and 48; 18 is the default, decision 68's size, so every existing golden is unchanged) and an optional wrap width, never narrower than one em. Other annotation kinds always carry the Standard format.
    - **Layout in the core:** `LabelLayout` (FrisketCore, `Labels.swift`) breaks lines once with the pinned `HelveticaNeue-Bold` at the label's own size (`CTTypesetterSuggestLineBreak`, at the wrap width or only at line breaks), and gives the box: the text's width (the wrap width when set) and height, plus the style's margin. `AnnotationPainter` draws those same line ranges with the font scaled, so the preview and every output wrap alike at any scale.
    - **Styles:** Standard is decision 68's label: a 2 px white plate stroke, then ink glyphs. Outlined: the plate strokes the glyphs wider than the outline, the ink strokes an outline of size/9 pt outside the glyphs, then the glyphs fill white. Box: the plate fills the box 1 px larger, the ink fills the box (padding 0.3 em across, 0.15 em down, snapped outward to whole pixels like a shape), then the glyphs fill white, or black on a light ink (luma above 0.6). The box is a label's background, not a filled shape: it always carries text and is never offered as concealment.
    - **Width handle:** a label has one handle, `trailing`, on the middle of its box's right edge. Dragging it sets the wrap width ("Resize Label"); size and style changes are "Restyle Label" (`MarkEditor.relabelSelection`).
    - **Typing:** `LabelSession` (core) writes every change of the typed text into the edits at once, so the canvas shows the label as it will be delivered; while the text is blank no label is in the edits. The session is one undo step on the window's undo manager ("Label", or "Edit Label" for an existing label) through `UndoableEdits.apply(_:named:coalescing:)`, which folds later changes with the same key into the step until anything else is applied, undone or redone. An undo during typing ends the session.
    - **AppKit, kept thin:** the Text tool's click starts a session; a clear `NSTextView` over the label holds the caret and selection (its own text is invisible), with smart quotes, dashes, replacement and correction off so the characters are exactly the typed ones, and its own undo off so typing uses the edits' undo. The canvas draws a dashed box around the label. Return, Tab, Esc or a click elsewhere end typing; Return is never Done, and Esc on a blank label leaves nothing. Option-Return types a line break. A click with the Text tool on a label edits it; when typing ends, the label is selected, ready for its handle and menus. The toolbar label text field is removed; the Text tool's contextual controls are a Label Size and a Label Style menu, which set new labels' format and restyle the label being typed or the selected one.
    - **Proof:** `LabelTests`: `v2.1 $4.99 -10%` typed one character at a time is stored and laid out exactly, each character with its own glyph in the pinned font (no fallback, no case folding); the session is one undo step; a blank label leaves nothing; the handle wraps and never goes below one em; per-style pixel goldens at glyph stems and box corners; and preview equals delivered byte for byte for every style, size and wrap at 1× and 2× with a crop and a Solid redaction.

85. **Slim repository checks; diagnostics through os.Logger (implementer, 2026-09-25, ticket 80; O12, O17, decisions 25, 54 and 57):**
    - **What `Checks/check_repository.py` keeps:** the six named invariants, as checks `finalization` (1), `app-writes` (2), `storage-pixels` (3), `core-io` (4), `input-monitoring` (5) and `network` (6); the core import allowlist inside `core-io` (DA-1's frameworks; the storage adapter also GRDB and Darwin), which CLAUDE.md names; and ticket 77's `app-sources`. `ci.sh` runs them in one process, plus `--self-test`, which fails unless every check has a passing and a failing fixture. `check_drift.py` (retired terms) and `ci.sh --defects` are unchanged.
    - **Deleted:** the `dependencies` (manifest dump), `imports` (folded into `core-io`), `identity` and `provenance` (Snapzy; with the empty `docs/ported-files.json`), `diagnostics`, `silgen` and `modals` checks; the storage eager-write, the capture-case and the clipboard-declaration regexes; the one-hot-key count; and the second run of every check inside the Swift suite. D22's `@_silgen_name` and DA-5's NSAlert rule are no longer fenced: both are fixed, and review plus `retired-terms.tsv` catch a return. The clipboard invariant stays tested at the adapter (`copyWritesPNGAndConcealedMarkerWithCurrentHostOnly`), Solid redaction by the renderer and editor tests.
    - **Diagnostics:** `LocalDiagnosticLog` (in memory, never read) and `DiagnosticRecord` are gone. The closed `DiagnosticEvent` and `DiagnosticSink` stay in the core; the app injects the adapters' `SystemDiagnosticLog`, one `os.Logger` line per event (`notice`, or `error` with a code), marked public because it holds only enum raw values. The unified log keeps and expires entries; Frisket writes no log file (decision 25 holds). The core, which may not import `os`, drops events when no sink is injected. The opt-in latency stdout rows stay: `Tools/Performance/baseline.py` reads them.
    - **Dead code removed:** `FrisketCore.swift` (comments only), the empty `discardSelectionPreviews` (the platform had no previews; its fakes and preview assertions go too), `SystemScreenshotHotkeys.familyIdentifiers` (no reader), `ClipboardImage.currentHostOnly` and `.concealed` (constants only tests read; the adapter sets both), `HistoryEntry.imageLocation` and `.thumbnailLocation` (now a test-target extension), `Codable` on the diagnostic types, `build-snapzy.sh` and the `snapzy` tool in `measure.py`. Kept although it looks empty: `DragSessionEvents` (the drag adapter's sequencing calls it).

86. **The style bar: contextual style controls under the toolbar (implementer, 2026-09-25, ticket 92; D29, decisions 54, 57, 59, 61 and 82–84):**
    - **Where:** the editor toolbar holds only the tools, Undo and Close. The Solid redaction colour swatches, the Arrow style and Line width menus and the Label size and Label style menus move to the style bar, a 32-point row across the top of the content area, above the hint line. Like the action bar (D5, ticket 53), it lives in the content area, so no control can fall into the toolbar's » overflow menu at any window width.
    - **What it shows:** only the controls that apply, as CleanShot does (decision 59): the selected mark's when it has any, or else the active tool's. Solid Redaction: the colour swatches; Arrow: arrow style and line width; Line and Shape: line width; Text: label size and style; Select, Crop, Blur and Magnify: nothing. A selected Solid redaction shows the swatches, a selected arrow its style and width, and so on; a selected Blur or Magnify box has no style, so the tool's controls stay. The rule is `StyleBar.controls(tool:selection:in:)` in FrisketCore, tested in `StyleBarTests`; the window only hides and shows the controls.
...
90. **Mark accessibility elements are owned by an adapter (implementer, 2026-09-25, ticket 96; D31, ticket 84):**
...
95. **Live drag feedback through the preview renderer (implementer, 2026-09-25, ticket 97; decisions 54, 57, 61, 75 and 77):**
    - **Provisional edits:** on every drag event the editor computes the edits mouse-up would commit and renders them with `CapturePreview.render`, the path Done's output matches. A held mark uses `MarkEditor.provisional(_:fromX:fromY:toX:toY:slop:)`, which shares its change with `release`; a drawing tool uses the same `applyDrag` call as mouse-up. So a move, resize or bend shows the mark with its real width, style, curve, head, ink and plate while dragging, and the selection outline follows it (`selectionOutline(in:)`). Provisional edits never enter `UndoableEdits`: the drag is still one commit and one undo step on mouse-up (decision 77). Crop keeps its dimming guide (the canvas does not resize mid-drag); the Text tool draws nothing.
    - **Guides:** the Solid redaction guide is the rendered redaction in the chosen fill, edged in white just outside it, one output pixel wide (at least one screen pixel). Shapes, arrows and lines have no overlay; the rendered mark is the guide. Blur and Magnify keep a thin outline so a flat area still shows the box. The 2 pt red line and box are gone.
    - **Throttling, not dirty rectangles:** the existing one-render-at-a-time loop (decision 75) renders only the latest provisional edits, so drag events never queue renders and the main actor only swaps images. Measured (release, this Intel Mac, `liveDragRendersEachPointWithinTheFrameBudget`, 60 points per drag, 6,016 × 3,384 capture, 2,048 × 1,152 preview): drawing a Solid redaction median 5.0 ms, slowest 11.3 ms; dragging a Curved arrow's tip median 5.5 ms, slowest 15.1 ms; moving a redaction over every edit kind with a whole-capture blur median 14.0 ms, slowest 16.9 ms. The 60 Hz frame is 16.7 ms. Rejected: dirty-rectangle rendering, because Blur, Magnify, antialiased plates and the downscale blocks read beyond a mark's box, so a partial repaint risks a preview that differs from the delivered image for a gain the numbers do not need.
    - **Tests:** `LiveDragTests` checks, at each point of a drag path for every mark kind and handle (move, corner, tail, tip, bend, wrap), that the provisional edits equal what `release` commits there (nil within the click slop), commit nothing and register no undo; that a whole live drag is one "Resize Arrow" undo step; and that the provisional preview equals the flattened delivery byte for byte, with a moving Grey redaction exactly Grey.

96. **Small fixes from the design audit (implementer, 2026-09-25, ticket 101):**
...
97. **The ink palette and light inks (implementer, 2026-09-25, ticket 99; decisions 54, 57, 68, 81, 83, 84, 86 and 92):**
    - **The palette:** `DocumentAnnotation.palette` in FrisketCore holds six `InkColour`s, Apple's system hues: Red #FF3B30 (the existing default ink, so every golden is unchanged), Yellow #FFCC00, Blue #007AFF, Green #34C759, Black and White. `DocumentAnnotation.inkName(_:)` names a palette ink. There is no free colour picker.
    - **The control:** `StyleControl.inkColour` is first in the style bar for the Arrow, Line, Shape and Text tools and for a selected arrow, line, shape or label; never for Solid redaction, Crop, Blur, Magnify or Select, nor for a selected redaction or effect (`StyleBar.controls`, `StyleBarTests`). Six swatch buttons, "Ink colour: Red" and so on, in a group named "Ink colour", as ticket 88's redaction swatches. A choice becomes the ink of every ink tool (one editor-wide ink, as CleanShot does), and recolours the label being typed (`LabelSession.recolour`, "Restyle Label") or else the selected annotation (`MarkEditor.recolourSelection`, "Restyle Arrow" and so on). Not remembered between editors; ticket 100 does that (decision 93).
    - **The plate stays white for every ink.** Decision 92 keeps decision 68's white plate, and nothing technical argues against it: the plate only widens the stroke outside, the ink inside is exact in every colour (`aShapeIsDrawnExactlyInEachInk`), and White or Yellow ink on a light background is the user's visible choice, which another swatch fixes. Rejected: a dark plate under light inks. It would amend a product decision and change how White and Yellow marks look, so it is Prateek's call, not a technical one.
    - **Letters in a light ink are black.** A Box label already drew black letters on a light ink (luma above 0.6). An Outlined label filled its letters white, so in White ink the letters vanished into their outline. Both styles now use one rule, `AnnotationPainter.letterColour(on:)`: white letters on Red, Blue, Green and Black, black on Yellow and White. Red's letters are unchanged, so existing goldens hold (`anOutlinedLabelInALightInkHasBlackLetters`).
    - **Proof:** `InkPaletteTests` recolours a shape, an arrow, a line and a label in each non-default ink: one "Restyle <noun>" undo step, reverted by one undo, and the preview equals the flattened delivery byte for byte. The live row `editor-ink-colour` draws a Shape in Blue over the pattern's red quadrant and counts #007AFF pixels in the copy (new `meter colour` mode); it is uncalibrated until its first live run.

98. **Remembered editor styles, the label corner handle and the reopened onboarding's Close (implementer, 2026-09-25, ticket 100; decisions 54, 57, 80, 93 and 96):**
    - **Model:** `EditorStyles` in FrisketCore holds what the style bar sets for new marks: the Solid redaction colour, the ink, the Arrow tool's style, the Arrow, Line and Shape tools' line widths (each its own, as the editor already kept them), and the Text tool's label size and style. `load` reads each value through its own `PreferenceKey` (`editorRedactionColour` … `editorLabelStyle`, eight keys) and `store` writes them; colours are stored as `RRGGBB`, styles by name, widths and sizes in points. The core never touches UserDefaults (the `core-io` check): the editor passes closures over `UserDefaults.standard`.
    - **Validation per value:** a stored value that is not offered (a colour outside its palette, the Line style as the Arrow style, a width or size not in the menu, the wrong type, NaN) falls back to its own default, and the other values load as stored. A remembered redaction colour is therefore always an opaque palette colour; Solid redaction's guarantees are unchanged (decision 93).
    - **What counts as the last choice:** what the tools would draw next. The editor stores the styles whenever a style control changes the tools; restyling a selected mark while another tool is active changes only that mark, as before, so it changes nothing remembered. A label's wrap width belongs to that label and is not kept. The editor opens with the stored styles; decision 82's "each editor opens with black" and decision 97's "not remembered between editors" are amended by decision 93.
    - **Label corner handle (ticket 101's fifth criterion, audit item 7):** a selected label has a bottom-right handle next to its side handle. Dragging it scales the label's size by the drag's projection on the box's diagonal, from the box's top-left corner, rounded to whole points within 6–144; a wrap width scales by the same ratio, so the text wraps at the same words. It is a `MarkChange.resize(.bottomRight, …)`, so the live drag previews through `MarkEditor.provisional` (decision 95) and mouse-up is one "Resize Label" undo step. A size the size menu does not offer shows as its own item in the menu.
    - **Reopened onboarding:** `OnboardingContent.current(for:)` keeps the first-launch copy while onboarding is incomplete ("Later", shown again on the next launch). Once it is complete, reopening from "What Frisket Stores…" shows "Close", whose VoiceOver label is "Close onboarding. Reopen it with What Frisket Stores… in the Frisket menu bar menu", because a completed onboarding never returns on launch.

99. **Choosing a drawing tool drops the selection (coordinator, 2026-09-25; decisions 81 and 86):**
...
101. **The first run keeps its Request button (implementer, 2026-09-25, ticket 103):**
