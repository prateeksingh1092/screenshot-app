...
- [ ] Tier 1: a fault injected at each named commit point, rebuild over the same directory, recovery run twice, invariants and size totals match disk.
- [ ] Tier 2: a helper executable kills itself at a chosen point, at least one case per point.
- [ ] A coverage check fails if a commit point exists without a matching case; later tickets that add points extend the list.
