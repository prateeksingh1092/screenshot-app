...
- [x] One Swift Testing test runs green under `swift test` with the Command Line Tools; if it can't, the blocker is reported accurately and the test is marked Xcode-only.
...
- [x] Running the build needs Prateek's approval at the time.
