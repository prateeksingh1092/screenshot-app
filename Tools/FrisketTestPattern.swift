...
@main enum FrisketTestPattern {
    @MainActor static func main() {
        ...
        guard args.count == 2, ["--show", "--show-all", "--show-full-screen", "--full-screen", "--show-window"].contains(args[1]) else {
            fputs("Usage: FrisketTestPattern --show | --show-all | --show-full-screen | --full-screen | --show-window | --verify /path/to/pasted.png 1|2 | --verify-full /path/to/pasted.png WIDTH HEIGHT 1|2 | --verify-redacted /path/to/image.png 1|2\n", stderr)
            exit(2)
        }
